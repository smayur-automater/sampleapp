    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, height: '56px', background: '#fff', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
        ...
        <button
          ...
          style={{ padding: '6px 14px', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#fff', fontSize: '13px', color: '#64748b', cursor: 'pointer', fontWeight: '500' }}
        >
          Sign out
        </button>
      </header>
      ...
      {/* Page content */}
      <main style={{ flex: 1, paddingTop: '56px', paddingBottom: '68px' }}>
        {children}
      </main>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, height: '68px', background: '#fff', borderTop: '1px solid #e2e8f0', display: 'flex' }}>
        {TABS.map(({ path, label, icon: Icon }) => {
          ...
          return (
            <button key={path} onClick={() => router.push(path)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '4px', border: 'none', background: 'none', cursor: 'pointer', color: active ? '#2563eb' : '#94a3b8', transition: 'color 0.15s' }}>
              <Icon active={active} />
