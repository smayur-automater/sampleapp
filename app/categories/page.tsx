...
export default function CategoriesPage() {
  ...
  async function save() {
    ...
    setSaving(true)
    if (editing) {
      await supabase.from('categories').update({ name: form.name, emoji: form.emoji, color: form.color }).eq('id', editing.id)
    } else {
      await supabase.from('categories').insert({ user_id: user.id, name: form.name, emoji: form.emoji, color: form.color })
    }
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '20px 16px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '56px 24px', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '20px' }}>
            ...
            <div style={{ fontWeight: '600', fontSize: '16px', color: '#374151', marginBottom: '6px' }}>No categories yet</div>
            <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '20px' }}>Add categories like Medical, School, Sports…</div>
            <button onClick={openAdd} style={{ padding: '10px 20px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Add first category</button>
          </div>
        ) : (
            ...
            {cats.map(cat => (
              <div key={cat.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                  <div style={{ width: '44px', height: '44px', borderRadius: '13px', background: cat.color + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', flexShrink: 0 }}>
                    {cat.emoji}
                  </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div style={{ background: '#fff', borderRadius: '24px 24px 0 0', padding: '24px', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#0f172a' }}>{editing ? 'Edit category' : 'Add category'}</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center' }}>
                <div style={{ width: '72px', height: '72px', borderRadius: '20px', background: form.color + '20', border: `3px solid ${form.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px' }}>
                  {form.emoji}
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: '6px' }}>
                  {EMOJIS.map(em => (
                    <button key={em} type="button" onClick={() => setForm({ ...form, emoji: em })} style={{ padding: '10px', fontSize: '20px', background: form.emoji === em ? '#eff6ff' : '#f8fafc', border: form.emoji === em ? '2px solid #2563eb' : '1px solid #e2e8f0', borderRadius: '10px', cursor: 'pointer' }}>{em}</button>
                  ))}
                </div>
              </div>
              <div>
